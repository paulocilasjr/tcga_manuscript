...
# Removed unused imports: zipfile, PIL.Image, cv2
...
def fuzzy_merge(metadata, embeddings, threshold=90):
    ...
    for _, metadata_row in metadata.iterrows():
        ...
        # Get the best match from embeddings
        ...
        if best_match and best_match[1] >= threshold:
            ...
            # Create a dictionary for the merged row to avoid index issues
            merged_dict = {
                **metadata_row.to_dict(),  # Metadata columns
                **{k: v for k, v in matching_row.to_dict().items() if k != "sample_name"}  # Embedding columns, excluding duplicate sample_name
            }
    ...
    if matched_samples:
        ...
        matched_df = pd.DataFrame(columns=list(metadata.columns) + [col for col in embeddings.columns if col != "sample_name"])
    return matched_df
...
def bag_by_sample(df, pooling_method, bag_size):
    ...
    for _, group in df.groupby("sample_name"):
        embeddings = group.iloc[:, 3:].values  # Assuming embeddings start at column 3
...
def bag_processing(embeddings, metadata, pooling_method, balance_enforced=False, bag_sizes=[3, 5], repeats=1, ludwig_format=False, by_sample=False):
    ...
    for split in merged_data['split'].unique():
        if by_sample:
            merged_data["instance_idx"] = merged_data.groupby("sample_name").cumcount()
