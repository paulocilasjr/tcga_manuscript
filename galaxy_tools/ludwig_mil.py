...
def fuzzy_merge(metadata, embeddings, threshold=90):
    ...
    for _, metadata_row in metadata.iterrows():
        ...
        # Get the best match from embeddings for the current sample_name
        best_match = process.extractOne(sample_name, embeddings["sample_name"].tolist())
        
        if best_match and best_match[1] >= threshold:  # If match score is above the threshold
            # Find the matching embedding row by sample_name
            matching_row = embeddings[embeddings["sample_name"] == best_match[0]].iloc[0]
            # Append the matched data
            merged_row = pd.concat([metadata_row, matching_row], axis=0)
            matched_samples.append(merged_row)
        else:
            logging.warning(f"No match found for sample_name: {sample_name}")

    # Convert matched rows back into a DataFrame
...
def bag_processing(embeddings,
    ...
    # Apply fuzzy matching between metadata and embeddings
    merged_data = fuzzy_merge(metadata, embeddings)

    for split in merged_data['split'].unique():
        if by_sample:
            merged_data["instance_idx"] = merged_data.groupby("sample_name") \
                                                    .cumcount()
            merged_data["instance_idx"] = merged_data.groupby("sample_name") \
                                                    .cumcount()
